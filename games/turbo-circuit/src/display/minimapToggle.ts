export function enableMinimapToggle(minimap: HTMLElement) {
  ...
  style.textContent = `.turbo-minimap{pointer-events:auto;cursor:pointer;transition:opacity .18s,width .18s,background .18s}.turbo-minimap[data-expanded="true"]{width:min(48cqw,65cqh,240px);background:#080b10ed;box-shadow:0 8px 28px #0009}`;
  minimap.append(style);
  const toggle = () => {
    minimap.dataset.expanded = minimap.dataset.expanded === "true" ? "false" : "true";
    minimap.setAttribute("aria-expanded", minimap.dataset.expanded);
  };
