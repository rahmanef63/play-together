...
const MESSAGES: Readonly<Record<string, string>> = {
  INVALID_CREDENTIALS:
    "Email or password is incorrect. Try again, or use Google if that is how you created your account.",
...
export function authErrorMessage(reason: unknown, operation: AuthOperation = "signIn"): string {
  ...
  const requestId = /\[Request ID:\s*([a-f0-9]{16,32})\]/i.exec(raw)?.[1];
  return requestId ? `${message} Support reference: ${requestId.toLowerCase()}.` : message;
}
